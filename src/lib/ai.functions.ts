...
// ⚠️ TODO: Set your Gemini API key as an environment variable.
// Add GEMINI_API_KEY to your server environment (e.g. via Secrets or .env on your hosting).
...
export const generateFollowUp = createServerFn({ method: "POST" })
  .inputValidator((input: { leadName: string; company: string; lastMessage: string; notes: string; tone: string }) => input)
  .handler(async ({ data }) => {
...
export const summarizeConversation = createServerFn({ method: "POST" })
  .inputValidator((input: { conversationText: string; leadName: string; company: string }) => input)
  .handler(async ({ data }) => {
...
➡️ Next Step: (recommended action)`;
    ...
    const systemInstruction = "You are a sales intelligence assistant. Provide clear, actionable summaries of sales conversations.";
...
// --- Smart Suggestions ---
export const getSmartSuggestions = createServerFn({ method: "POST" })
  .inputValidator((input: { leadName: string; company: string; status: string; notes: string; messageHistory: string }) => input)
  .handler(async ({ data }) => {
...
🗓️ Timing: (when to follow up)`;
    ...
    const systemInstruction = "You are an AI sales advisor. Give specific, actionable suggestions based on lead data. Be direct and practical.";
